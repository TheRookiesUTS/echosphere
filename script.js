...
const NASA_API_KEY = 'EvxEEMh4PuPJA1SRT0OHfRct1pNBNZcvitY9opPw';
...
async function fetchNASAEarthImagery(lat, lng) {
    const url = `${NASA_BASE_URL}/planetary/earth/imagery?lon=${lng}&lat=${lat}&dim=0.1&api_key=${NASA_API_KEY}`;
    ...
    try {
        ...
        console.log('NASA Earth imagery API error:', error);
        return null;
    }
}

async function fetchNASAAirQuality(lat, lng) {
    // Using NASA's OMI data simulation
    const url = `${NASA_BASE_URL}/planetary/earth/assets?lon=${lng}&lat=${lat}&date=2024-01-01&dim=0.1&api_key=${NASA_API_KEY}`;
    ...
    try {
        ...
        console.log('NASA air quality API error:', error);
...
function analyzeSelectedArea() {
    ...
    const analysis = generateAreaAnalysis(selectedAreaData);
